...
| `ANTHROPIC_API_KEY`         | **yes** | Provider key for independent memo analysis (`POST /api/independent-analysis`, model `claude-sonnet-4-6`). **Server-only** — never sent to the browser, never embedded in the static export. Bound on Railway from the vault. Surfaced as the informational `anthropic` boolean on `/ready` (never gates readiness). When unbound, the API falls back to a deterministic offline synthesizer that still cites real `kpi_values`. |
...
| `SMTP_URL`                  | **yes** | Optional third magic-link backend: a full SMTP submission URL, `smtps://user:pass@host:465` (implicit TLS) or `smtp://user:pass@host:587` (STARTTLS). The credential most operators already hold (their own mail domain), so it is the easiest way to arm real delivery with no third-party signup. The dependency-free SMTP client lives in `apps/api/src/mailer.js` (node:net/node:tls only — no lockfile/native surface). Contains a password → **secret**. |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_SECURE` | `SMTP_PASS` **yes** | Optional discrete form of `SMTP_URL` (use one or the other). `SMTP_PORT` defaults to `587`; `SMTP_SECURE=true` (or port `465`) selects implicit TLS, otherwise the client upgrades via STARTTLS when the server advertises it. Binding any of these lights up `POST /auth/v1/otp` exactly like `RESEND_API_KEY`. |
| `SMTP_DIRECT`               | no      | Zero-credential magic-link backend (`true`/`1`). The mailer resolves the recipient domain's MX and delivers straight to it on port 25 (STARTTLS when offered, no AUTH) — the one delivery path that needs **no secret and no third-party account**, so it is armable purely as a non-secret flag. **Opt-in, off by default** (it can send real mail to a real recipient domain). Only works where outbound port 25 is permitted; many hosts (Railway included) block it, in which case `POST /auth/v1/otp` fails honestly with `502` rather than a false "check your email". Not a password → **non-secret**. |
| `AUTH_EMAIL_FROM`           | no      | Optional verified sender for magic-link emails (defaults to `Boardroom <login@theimagegroup.com>`). |
| `FOUNDER_TEST_EMAIL`        | no      | Optional. Extra founder invite address (case-insensitive). Also used by e2e/probes. Default built-in invite is `founder.e2e@boardroom.test`. |
| `BOARD_TEST_EMAIL`          | no      | Optional. Extra board invite address. Default built-in invite is `board.e2e@boardroom.test`. |
| `AUTH_INVITE_ALLOWLIST`     | no      | Optional comma/whitespace-separated list of additional invited member emails. Self-hosted `POST /auth/v1/otp` only mints grants for invited addresses (defaults + these env lists). Unknown emails get a silent 200 with no `action_link` — no self-signup. |
